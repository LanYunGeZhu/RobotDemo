...
- (UILabel *)dayLabel {
    if (!_dayLabel) {
        _dayLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 20, 15)];
...
typedef NS_ENUM(NSUInteger, FDCalendarMonth) {
    FDCalendarMonthPrevious = 0,//上月
    FDCalendarMonthCurrent = 1,//当前月
    FDCalendarMonthNext = 2,//下月
};
...
- (NSDate *)dateOfMonth:(FDCalendarMonth)calendarMonth WithDay:(NSInteger)day {
    ...
    switch (calendarMonth) {
        case FDCalendarMonthPrevious:
            date = [self previousMonthDate];//上月
            break;
            
        case FDCalendarMonthCurrent://当月
            date = self.date;
            break;
            
        case FDCalendarMonthNext://下月
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSInteger firstWeekday = [self weekdayOfFirstDayInDate];//当月第一天是星期几
